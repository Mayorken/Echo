 ...
 * lib/crypto.js
 *
 * Real AES-256-GCM encryption for Echo context files, replacing the
 ...
 * where this SDK actually runs inside an AI tool's integration), and falls
 * back to Node's built-in `crypto` module so the same file is testable
 ...
 * key means losing access to the context file; this module doesn't attempt
...
async function encrypt(plaintext, keyBytes) {
  if (!(plaintext instanceof Uint8Array)) {
    throw new Error('encrypt: plaintext must be a Uint8Array');
  }
  if (!(keyBytes instanceof Uint8Array) || keyBytes.length !== 32) {
    throw new Error('encrypt: keyBytes must be a 32-byte Uint8Array');
  }
...
async function decrypt(packed, keyBytes) {
  if (!(packed instanceof Uint8Array)) {
    throw new Error('decrypt: packed must be a Uint8Array');
  }
  if (!(keyBytes instanceof Uint8Array) || keyBytes.length !== 32) {
    throw new Error('decrypt: keyBytes must be a 32-byte Uint8Array');
  }
  // Minimum length: IV (12) + at least 1 byte ciphertext + auth tag (16)
  if (packed.length < IV_LENGTH + 1 + 16) {
    throw new Error('decrypt: packed data is too short to contain valid ciphertext');
  }
