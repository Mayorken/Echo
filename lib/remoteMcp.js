...
function isStorageFundingError(error) {
  return /(PaymentsService|fundSync|insufficient balance|failed to execute fund operation)/i.test(error && error.message || '');
}

async function createRemoteMcpServer({ client, userAddress, recoveryKey, onActivity = () => {} }) {
  ...
  server.registerTool('echo_save_context', {
    ...
    try {
      ...
      if (isStorageFundingError(error)) {
        return {
          isError: true,
          content: [{ type: 'text', text: 'Echo storage is temporarily unavailable because the platform storage reserve needs funding. You were not charged and no payment action is required from you. Please try again after the Echo operator replenishes storage.' }],
        };
      }
      throw error;
    }
...
async function handleRemoteMcp(req, res, options) {
  ...
  if (options.onActivity) options.onActivity('connected');
...
module.exports = { createRemoteMcpServer, handleRemoteMcp, mergeContext, containsSensitiveFields, isStorageFundingError };
